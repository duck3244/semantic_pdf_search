...
import fitz  # PyMuPDF — 한글 CID/임베디드 폰트를 PyPDF2보다 정확히 추출
...
class PDFService:
    ...
    def extract_text_from_pdf(self, pdf_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(total_pages):
                ...
                # "text" 추출: 페이지 자연 순서대로 줄바꿈 포함 plain text
                text = doc[i].get_text("text") or ""
                if not text.strip():
                    continue

                # 빈 줄(이중 개행) 기준으로 단락 분할
                for j, para in enumerate(text.split('\n\n')):
                    para = para.strip()
                    if len(para) < 20:  # 너무 짧은 라벨/페이지 번호 제외
                        continue
                    text_chunks.append({
    ...
    def process_pdf_files(self, pdf_files: List[str]) -> Tuple[pd.DataFrame, np.ndarray, List[Dict[str, Any]]]:
        ...
        # 임베딩 생성 (EmbeddingModel 내부에서 배치 처리)
        logger.info(f"Generating embeddings for {len(chunks_df)} text chunks")
        start_time = time.time()

        embeddings = self.embedding_model.get_embeddings(chunks_df['text'].tolist())
    ...
    def save_uploaded_pdf(self, file_content: bytes, filename: str) -> str:
        """
        업로드된 PDF 파일 저장. path traversal 방지를 위해 basename만 사용.
        """
        safe_name = os.path.basename(filename or "").strip()
        if not safe_name or safe_name in (".", ".."):
            raise ValueError("Invalid filename")

        if os.path.exists(os.path.join(PDF_DIR, safe_name)):
            base, ext = os.path.splitext(safe_name)
            safe_name = f"{base}_{os.urandom(4).hex()}{ext}"

        file_path = os.path.join(PDF_DIR, safe_name)
        # 최종 경로가 PDF_DIR 하위인지 재확인 (방어적)
        if not os.path.abspath(file_path).startswith(os.path.abspath(PDF_DIR) + os.sep):
            raise ValueError("Resolved path escapes PDF directory")

        with open(file_path, 'wb') as f:
            f.write(file_content)

        logger.info(f"Saved uploaded PDF: {safe_name}")
        return file_path
